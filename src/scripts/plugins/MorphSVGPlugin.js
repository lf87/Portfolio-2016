...
(_gsScope._gsQueue || (_gsScope._gsQueue = [])).push(function() { "use strict";
    var a = Math.PI / 180,
        ...
        m = function(a) { window.console && console.log(a) },
        n = function(b, c) {
        ...
        o = function(c, d, e, f, g, h, i, j, k) {
            if (c !== j || d !== k) { e = Math.abs(e), f = Math.abs(f);
                var l = g % 360 * a,
                    m = Math.cos(l),
                    o = Math.sin(l),
                    p = (c - j) / 2,
        ...
        p = function(a) {
            var k, l, n, p, q, r, s, t, u, v, w, x, y, b = (a + "").replace(f, function(a) {
                    var b = +a;
                    return 1e-4 > b && b > -1e-4 ? 0 : a }).match(c) || [],
                d = [],
                e = 0,
                g = 0,
                h = b.length,
            ...
            for (k = 0; h > k; k++)
                ...
                e = s[i - 2], g = s[i - 1], k += 7 } else m("Error: malformed path data: " + a);
        ...
        F = function(a, b, c) {
            return b || c || a || 0 === a ? function(d) { E(d, a, b, c) } : E },
        G = function(a, b) {
            if (!b) return a;
        ...
        J = function(a) {
            return isNaN(a) ? I : function(b) { I(b), b[1] = G(b[1], parseInt(a, 10)) } },
        K = function(a, b) {
        ...
        O = _gsScope._gsDefine.plugin({ propName: "morphSVG", API: 2, global: !0, version: "0.8.4", init: function(a, b, c) {
                var d, f, g, n, o;
                return "function" != typeof a.setAttribute ? !1 : l ? (d = a.nodeName.toUpperCase(), o = "POLYLINE" === d || "POLYGON" === d, "PATH" === d || o ? (f = "PATH" === d ? "d" : "points", ("string" == typeof b || b.getBBox || b[0]) && (b = { shape: b }), n = M(b.shape || b.d || b.points || "", "d" === f, a), o && e.test(n) ? (m("WARNING: a <" + d + "> cannot accept path data. " + N), !1) : (n && (this._target = a, a.getAttributeNS(null, "data-original") || a.setAttributeNS(null, "data-original", a.getAttribute(f)), g = this._addTween(a, "setAttribute", a.getAttribute(f) + "", n + "", "morphSVG", !1, f, "object" == typeof b.precompile ? function(a) { a[0] = b.precompile[0], a[1] = b.precompile[1] } : "d" === f ? F(b.shapeIndex, b.map || O.defaultMap, b.precompile) : J(b.shapeIndex)), g && (this._overwriteProps.push("morphSVG"), g.end = n, g.endProp = f)), l)) : (m("WARNING: cannot morph a <" + d + "> SVG element. " + N), !1)) : (window.location.href = "http://" + j + k + "?plugin=" + i + "&source=" + h, !1) }, set: function(a) {
                var b;
    ...
    O.pathFilter = E, O.pointsFilter = I, O.subdivideRawBezier = q, O.defaultMap = "size", O.pathDataToRawBezier = function(a) {
        return p(M(a, !0)) }, O.equalizeSegmentQuantity = D, O.convertToPath = function(a, b) { "string" == typeof a && (a = g.selector(a));
        for (var c = a && 0 !== a.length ? a.length && a[0] && a[0].nodeType ? Array.prototype.slice.call(a, 0) : [a] : [], d = c.length; --d > -1;) c[d] = L(c[d], b !== !1);
        return c }, O.pathDataToBezier = function(a, b) {
        var e, f, h, i, j, k, l, m, c = p(M(a, !0))[0] || [],
            d = 0;
        if (b = b || {}, m = b.align || b.relative, i = b.matrix || [1, 0, 0, 1, 0, 0], j = b.offsetX || 0, k = b.offsetY || 0, "relative" === m || m === !0 ? (j -= c[0] * i[0] + c[1] * i[2], k -= c[0] * i[1] + c[1] * i[3], d = "+=") : (j += i[4], k += i[5], m && (m = "string" == typeof m ? g.selector(m) : m && m[0] ? m : [m], m && m[0] && (l = m[0].getBBox() || { x: 0, y: 0 }, j -= l.x, k -= l.y))), e = [], h = c.length, i)
            for (f = 0; h > f; f += 2) e.push({ x: d + (c[f] * i[0] + c[f + 1] * i[2] + j), y: d + (c[f] * i[1] + c[f + 1] * i[3] + k) });
        else
            for (f = 0; h > f; f += 2) e.push({ x: d + (c[f] + j), y: d + (c[f + 1] + k) });
        return e } }), _gsScope._gsDefine && _gsScope._gsQueue.pop()();
